...
//   useEffect(() => {
//     axios
//       .get(`http://localhost:4000/product/get/${est_id}`)
//       .then((res) => {
//           const { est_no } = res.data.Result[0];
//           setInvoiceNumber(est_no);
//           const { p_description } = res.data.Result[0];
//           setInpVal(c_name);
//           const { p_hsn_code } = res.data.Result[0];
//           setItemHSN(p_hsn_code);
//           const { p_item_type} = res.data.Result[0];
//           setItemType(p_item_type);
//           const { p_category } = res.data.Result[0];
//           setCategory(p_category);
//           const { p_purchase_price } = res.data.Result[0];
//           setPurchasePrice(p_purchase_price);
//           const { p_selling_price } = res.data.Result[0];
//           setSalesPrice(p_selling_price);
//           const { p_master_tax } = res.data.Result[0];
//           setTaxRate(p_master_tax);
//           const { p_purchase_tax } = res.data.Result[0];
//           setPurchaseTax(p_purchase_tax);
//           const { p_selling_tax } = res.data.Result[0];
//           setSellingTax(p_selling_tax);
//           const { p_product_unit} = res.data.Result[0];
//           setMeasuringUnit(p_product_unit);
//           const { p_opening_stock } = res.data.Result[0];
//           setOpeningStock(p_opening_stock);                   
//       })
