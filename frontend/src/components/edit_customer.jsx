...
function EditCustomer() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4000/customer/get/${c_id}`)
      .then((res) => {
          const customer = res.data.Result[0];
          setCustomerName(customer.c_name);
          setCustomerPhone(customer.c_phone);
          setCustomerEmail(customer.c_email);
          setCustomerOpeningBalance(customer.c_opening_balance);
          setCustomerGstIn(customer.c_gst);
          setCustomerPan(customer.c_pan);
          setCustomerType(customer.c_type);
          setCustomerCategory(customer.c_category);
          setBillingAddress(customer.c_billing_add);
          setShippingAddress(customer.c_shipping_add);
      })
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:4000/customer/update/${c_id}`,
        formData
      );
